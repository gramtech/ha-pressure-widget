 ...
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */function ft(t){return pt({...t,state:!0,attribute:!1})}const $t="ha-pressure-widget",_t="ha-pressure-widget-editor",mt=[{key:"stormy",label:"Stormy",from:950,to:980,color:"#8b1a1a"},{key:"rain",label:"Rain",from:980,to:1e3,color:"#b5651d"},{key:"change",label:"Change",from:1e3,to:1015,color:"#c9a227"},{key:"fair",label:"Fair",from:1015,to:1030,color:"#4a7a3c"},{key:"v-dry",label:"Very Dry",from:1030,to:1050,color:"#2f5f4f"}],gt={hPa:{major:10,minor:2},inHg:{major:.5,minor:.1}};function yt(t,e,s){if(s<=e)return-135;return 270*((Math.min(s,Math.max(e,t))-e)/(s-e))-135}function vt(t,e,s,i){const n=(i-90)*Math.PI/180;return{x:t+s*Math.cos(n),y:e+s*Math.sin(n)}}const At=33.8639,bt=new Set(["inhg","inHg","in"]),wt=new Set(["hpa","mbar","mb"]);function xt(t){return t/At}function Et(t,e){return"inHg"===e?xt(t):t}function St(t,e){return"inHg"===e?t*At:t}function Ct(t,e,s){return e===s?t:Et(St(t,e),s)}function Pt(t,e){const s=t.states[e]?.attributes?.unit_of_measurement;if(!s)return"hPa";const i=s.toLowerCase();return bt.has(i)||i.includes("inhg")?"inHg":(wt.has(i),"hPa")}const kt=100;function Ut(t,e,s){return mt.map(i=>{const n=Et(i.from,s),r=Et(i.to,s);if(r<=t||n>=e)return B``;const o=yt(n,t,e),a=yt(r,t,e);if(a<=o)return B``;const l=function(t,e,s,i,n){const r=vt(t,e,s,i),o=vt(t,e,s,n),a=n-i<=180?0:1;return`M ${r.x} ${r.y} A ${s} ${s} 0 ${a} 1 ${o.x} ${o.y}`}(kt,kt,86,o,a);return B`<path
      d=${l}
      stroke=${i.color}
      stroke-width=${9}
      fill="none"
    />`})}function Mt(t,e,s){const{major:i,minor:n}=gt[s],r=function(t,e,s,i){if(s<=0||i<=0||e<=t)return[];const n=[],r=i/1e3;for(let o=t;o<=e+r;o+=i){const a=Math.round(o/i)*i,l=Math.abs(a/s-Math.round(a/s))<r;n.push({value:a,angle:yt(a,t,e),major:l})}return n}(t,e,i,n);return r.map(t=>{const e=t.major?9:4,s=vt(kt,kt,79-e,t.angle),i=vt(kt,kt,79,t.angle),n=B`<line
      x1=${s.x} y1=${s.y} x2=${i.x} y2=${i.y}
      class=${t.major?"tick tick-major":"tick tick-minor"}
    />`;if(!t.major)return n;const r=vt(kt,kt,52,t.angle),o=B`<text x=${r.x} y=${r.y} class="tick-label" text-anchor="middle">${Math.round(10*t.value)/10}</text>`;return B`${n}${o}`})}function Ot(t){const{min:e,max:s,unit:i,value:n,pastValue:r,needleColor:o}=t;return B`
    <svg viewBox="0 0 200 200" class="dial" role="img" aria-label="Barometer dial">
      ...
      ${Ut(e,s,i)}
      ${Mt(e,s,i)}
      ${function(t,e,s){return mt.map(i=>{const n=Et(i.from,s),r=Et(i.to,s),o=(Math.max(n,t)+Math.min(r,e))/2;if(o<t||o>e)return B``;const a=yt(o,t,e),l=vt(kt,kt,75,a);return B`<text x=${l.x} y=${l.y} class="zone-label" text-anchor="middle">${i.label}</text>`})}(e,s,i)}
